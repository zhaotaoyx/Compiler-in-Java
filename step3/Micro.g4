...
{
	Symbol symbol = new Symbol($id.text, "STRING", $str.text, functionname);
	SymbolTable.symboltable.add(symbol);
}
	;
...
id_list:
	id 
{
	Symbol symbol = new Symbol($id.text, type, null, functionname);
	SymbolTable.symboltable.add(symbol);
}
	id_tail;
	...
	FUNCTION any_type id 
{
	...
	Symbol symbol = new Symbol("0" + $id.text, "FUNCTION", null, functionname);
	SymbolTable.symboltable.add(symbol);
}
	LBRACKET param_decl_list RBRACKET BEGIN func_body END;
...
if_stmt:
	IF
{
	String bn = "" + blocknumber;
	Symbol symbol = new Symbol(bn, "BLOCK", null, functionname);
	SymbolTable.symboltable.add(symbol);
	blocknumber++;
}
	...
	FOR
{
	...
	LBRACKET init_stmt SEMICOLON cond SEMICOLON incr_stmt RBRACKET decl aug_stmt_list ROF; 
